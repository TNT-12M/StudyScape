    ...
    try {
        switch ($action) {
            ...
            case 'list_questions':
                ...
                if (isAdmin() && $educationLevel === '') jsonOut(false, '管理员查看题库前请选择学段');
                ...
                $rows = dbFetchAll($db, "SELECT id, subject, question_type, category, education_level, content, options, correct_answer, explanation, difficulty, points, created_at FROM questions $whereSql ORDER BY id DESC LIMIT ? OFFSET ?", array_merge($params, [$page_size, $offset]));
